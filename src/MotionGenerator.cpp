...
void MotionGenerator::calculateTrapezoidalProfile(float posRef) {
  ...
  if (shape) // trapezoidal shape
  {
    if (t <= (tBrk + tAcc)) {
      ...
      pos = oldPos +
            signM * (-dBrk + dAcc + dVel + maxVel * (t - tBrk - tAcc - tVel) -
                     0.5 * maxAcc * (t - tBrk - tAcc - tVel) *
                         (t - tBrk - tAcc - tVel));
  ...
  } else // triangular shape
  {
    if (t <= (tBrk + tAcc)) {
      ...
      pos = oldPos +
            signM * (-dBrk + dAcc + velSt * (t - tBrk - tAcc) -
...
float MotionGenerator::getVelocity() { return vel; }

float MotionGenerator::getAcceleration() { return acc; }

void MotionGenerator::setMaxVelocity(float aMaxVel) { maxVel = aMaxVel; }

void MotionGenerator::setMaxAcceleration(float aMaxAcc) { maxAcc = aMaxAcc; }
